...
def main(**kwargs):
    ...
    if inference_config.load_type == 'peft':
        ...
        if inference_config.train_dataset == 'vanilla':
            ...
            model = LLM(ckpt, tensor_parallel_size=len(available_gpus), max_model_len=4096)
            macro_res = eval_inference(
                model,
                inference_config,
                eval_dataloader,
            ...
            for ckpt in sorted(ckpt_dirs):
                ...
                available_gpus = os.environ['CUDA_VISIBLE_DEVICES'].split(',')
                print("available gpus:", available_gpus)
                model = LLM(ckpt, tensor_parallel_size=len(available_gpus), gpu_memory_utilization=0.85, max_model_len=4096)
                macro_res = eval_inference(
                    model,
                    inference_config,
                    eval_dataloader,
