...
save_type="hf"
train_data_path="./dataset/bbh/bbh_all_data/all_task_train_right_wronghint_answer_B.json" # for the 1st step in edit
output_dir="../slm/$save_type/${last_dirname}_B"
mkdir -p "$output_dir"
ckpt_continue=None
max_words=1024
num_epochs=15
...
one_gpu=False
save_model=True
dist_checkpoint_root_folder="./output/fsdp/${last_dirname}_B" # will be used if using FSDP
...
mkdir -p "$dataset_dir"
mkdir -p "$dataset_dir/${last_dirname}_B"
log_file="$dataset_dir/${last_dirname}_B/log.txt"

export CUDA_VISIBLE_DEVICES="0,1,2,3,4,5,6,7"
torchrun --nnodes 1 --nproc_per_node 8 --master-port 29509 ./finetuning.py \
    --max_words $max_words \
