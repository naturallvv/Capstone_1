...
def compute_metrics(rank_model, rank_tokenizer, task_descs, task_names, teacher_responses, model_inputs, original_inputs, original_outputs, model_outputs, eval_preds, inference_config):
    
    stu_responses = copy.deepcopy(eval_preds)
    for i, x in enumerate(stu_responses):
        stu_responses[i] = x.split(original_inputs[i])[-1].lstrip('\n').lstrip('A:').strip().lstrip('Rationale:').strip()
    ...
    for i, x in enumerate(model_outputs):
        ...
        if 'Therefore, the answer is' in x:
            model_outputs[i] = x.split('Therefore, the answer is')[-1].split('.')[0].strip()
            ...
            model_outputs[i] = x.split('Answer:')[1].split('.')[0].strip()
            ...
            model_outputs[i] = x.split('the answer is')[1].split('.')[0].strip()
        else:
    ...
    results = {
        ...
        'model_ckpt_for_test': inference_config.saved_model_dir,
        'model train on': inference_config.train_dataset,
        'model test on': inference_config.test_dataset,
