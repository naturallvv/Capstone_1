...
def save_model_checkpoint(
    ...
    if rank == 0:
        ...
        save_name = cfg.dataset + "/epoch-" + str(epoch) + ".pt"
...
def load_fsdp_model_checkpoint(model, ckpt):
    model_checkpoint = torch.load(ckpt)
    # integrate into loaded model
    model.load_state_dict(model_checkpoint)
...
def save_model_to_hf(model, base_model, output_dir):
    tokenizer = AutoTokenizer.from_pretrained(
        base_model
    )
    tokenizer.save_pretrained(output_dir)
